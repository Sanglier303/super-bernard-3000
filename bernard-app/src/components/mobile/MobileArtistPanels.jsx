      ...
      <div style={{ width: '100%', maxHeight: '92vh', overflowY: 'auto', WebkitOverflowScrolling: 'touch', background: '#c0c0c0', borderTop: '2px solid #fff', boxShadow: '0 -2px 0 #404040', padding: '12px' }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <div style={{ ...sectionStyle, gap: '8px' }}>
            <div style={{ fontSize: '14px', fontWeight: 'bold', color: '#000080' }}>Les champs utiles tout de suite</div>
          ...
          <div style={{ ...sectionStyle, gap: '8px' }}>
            <div style={{ fontSize: '14px', fontWeight: 'bold', color: '#000080' }}>Présence & validation</div>
            <MobileField label="Instagram">
              <input name="instagram" defaultValue={artist.instagram || ''} style={inputStyle} />
            </MobileField>
            <MobileField label="Spotify / audio principal">
          ...
          <div style={{ ...sectionStyle, gap: '8px' }}>
            <div style={{ fontSize: '14px', fontWeight: 'bold', color: '#000080' }}>Contexte court</div>
            <MobileField label="Source type">
              <input name="source_type" defaultValue={artist.source_type || ''} style={inputStyle} />
            </MobileField>
            <MobileField label="Preuves / note courte">
...
export function MobileArtistDetail({ artist, onClose, onQuickEdit, onToggleValidation }) {
  ...
  const identityFacts = [
    ...
    { label: 'Performance', value: artist.type_performance || '—' },
    { label: 'Sous-genre', value: artist.sous_genre || '—' },
    { label: 'Source', value: artist.source_type || '—' },
  ]
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 1900, background: '#c0c0c0', overflowY: 'auto', WebkitOverflowScrolling: 'touch' }}>
      ...
      <div style={{ padding: '12px 12px 84px 12px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <div style={{ display: 'grid', gap: '8px' }}>
          <div style={{ fontSize: '14px', fontWeight: 'bold' }}>Repères rapides</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '8px' }}>
            {quickFacts.map(item => (
        ...
        <div style={{ display: 'grid', gap: '8px' }}>
          <div style={{ fontSize: '14px', fontWeight: 'bold' }}>Identité</div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '8px' }}>
        ...
        <div style={{ display: 'grid', gap: '8px' }}>
          <div style={{ fontSize: '14px', fontWeight: 'bold' }}>Contexte</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: '6px' }}>
              {links.map(link => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  style={{ color: '#000080', background: '#fff', border: '2px solid', borderColor: '#fff #808080 #808080 #fff', padding: '8px', textDecoration: 'none', minHeight: '44px', display: 'grid', alignContent: 'center' }}
                >
                  <b style={{ fontSize: '12px' }}>{link.label}</b>
                  <span style={{ fontSize: '10px', color: '#555', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{link.value}</span>
                </a>
              ))}
            </div>
          )}
