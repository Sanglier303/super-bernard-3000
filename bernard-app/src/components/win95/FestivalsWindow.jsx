...
export function FestivalsWindow({ festivals, loading, saveFestivals, onRefresh }) {
  ...
  const exportCSV = () => {
    setOpenMenu(null);
    let csv = "Nom,Periode,Duree,Lieu,Style,Instagram,Notes\n";
    festivals.forEach(f => {
      csv += `"${(f.nom||'').replace(/"/g,'""')}","${(f.periode||'').replace(/"/g,'""')}","${(f.duree||'').replace(/"/g,'""')}","${(f.lieu||'').replace(/"/g,'""')}","${(f.style||'').replace(/"/g,'""')}","${(f.instagram||'').replace(/"/g,'""')}","${(f.notes||'').replace(/"/g,'""')}"\n`;
    });
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a'); link.href = url; link.download = "export_festivals.csv"; link.click();
  ...
  const GRID = 'minmax(180px, 2fr) minmax(90px, 1fr) minmax(90px, 1fr) minmax(130px, 1.5fr) minmax(130px, 1.5fr) 100px';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: '#c0c0c0', position: 'relative' }}>
      ...
      <div className="win95-menubar" style={{ position: 'relative', zIndex: 200 }}>
        ...
        {openMenu === 'Fichier' && (
          <div style={{ position: 'absolute', top: '100%', left: '2px', background: '#c0c0c0', padding: '2px', ...raised, display: 'flex', flexDirection: 'column', minWidth: '150px' }}>
            <div className="win95-menu-item" style={{ fontSize: '11px', padding: '3px 12px' }} onClick={() => { setOpenMenu(null); handleAdd(); }}>Nouveau festival</div>
        ...
        {openMenu === 'Affichage' && (
          <div style={{ position: 'absolute', top: '100%', left: '44px', background: '#c0c0c0', padding: '2px', ...raised, display: 'flex', flexDirection: 'column', minWidth: '180px' }}>
            ...
            <div className="win95-menu-item" style={{ fontSize: '11px', padding: '3px 12px', display: 'flex', gap: '6px' }} onClick={() => { setShowAvatars(!showAvatars); setOpenMenu(null); }}>
              <span style={{ width: '12px' }}>{showAvatars ? '✔' : ''}</span> Afficher les affiches
            </div>
          </div>
        )}
        {openMenu === 'Recherche' && (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '4px 6px', borderBottom: '2px solid', borderColor: '#808080 #dfdfdf #dfdfdf #808080' }}>
        ...
        <input ref={searchInputRef} type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Nom, style, lieu..."
          ...
          <div style={{ width: '180px', borderRight: '2px solid', borderColor: '#808080 #dfdfdf #dfdfdf #808080', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
            ...
            <div style={{ overflowY: 'auto', flex: 1, background: '#fff', ...sunken, margin: '2px' }}>
              <div onClick={() => setActiveStyle(null)} style={{ ...winFont, padding: '2px 8px', cursor: 'default', display: 'flex', justifyContent: 'space-between', background: !activeStyle ? '#000080' : 'transparent', color: !activeStyle ? '#fff' : '#000' }}>
                <span>Tous</span><span style={{ color: !activeStyle ? '#adf' : '#666' }}>[{festivals.length}]</span>
              </div>
              {allStyles.map(s => {
                const count = festivals.filter(f => (f.style || '').toLowerCase().includes(s.toLowerCase())).length;
            ...
            <div style={{ borderTop: '2px solid', borderColor: '#808080 #dfdfdf #dfdfdf #808080', padding: '6px', background: '#c0c0c0' }}>
              <div style={{ ...sunken, background: '#fff', padding: '6px' }}>
                <div style={{ ...winFont, fontSize: '10px' }}>Festivals : <b>{festivals.length}</b></div>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, padding: 2 }}>
          <div style={{ overflowX: 'auto', flex: 1, display: 'flex', flexDirection: 'column', ...sunken, background: '#fff' }}>
            <div style={{ display: 'grid', gridTemplateColumns: GRID, ...raised, background: '#c0c0c0', flexShrink: 0 }}>
              {['Nom du Festival', 'Période', 'Durée', 'Lieu', 'Style Musical', 'Instagram'].map((h, i) => (
                ...
                return (
                  <div key={f.id} onDoubleClick={() => { setSelected(f); setEditingId(f.id); setAddEditOpen(true); }} onClick={() => setSelected(f)}
                    ...
                    <div style={{ ...winFont, padding: COL, borderRight: '1px dotted #ccc', display: 'flex', alignItems: 'center', gap: compactMode ? '4px' : '10px', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
                      ...
                      <span style={{ fontSize: FS }}>{f.nom || ''}</span>
                    </div>
                    <div style={{ ...winFont, padding: COL, borderRight: '1px dotted #ccc', fontSize: FS, color: isSel ? '#ddd' : '#555', display: 'flex', alignItems: 'center' }}>{f.periode || ''}</div>
                    <div style={{ ...winFont, padding: COL, borderRight: '1px dotted #ccc', fontSize: FS, display: 'flex', alignItems: 'center' }}>{f.duree || ''}</div>
                    <div style={{ ...winFont, padding: COL, borderRight: '1px dotted #ccc', fontSize: FS, color: isSel ? '#cce' : '#666', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', display: 'flex', alignItems: 'center' }}>{f.lieu || ''}</div>
                    <div style={{ ...winFont, padding: COL, borderRight: '1px dotted #ccc', fontSize: FS, color: isSel ? '#ddd' : '#555', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis', display: 'flex', alignItems: 'center' }}>{f.style || ''}</div>
                    <div style={{ padding: COL, display: 'flex', gap: '4px', alignItems: 'center' }}>
                      {f.instagram && <a href={f.instagram} target="_blank" onClick={e => e.stopPropagation()} style={{ ...raised, ...winFont, background: '#c0c0c0', color: '#000', textDecoration: 'none', padding: '1px 3px', fontSize: '9px', fontWeight: 'bold' }}>IG</a>}
                    </div>
                  </div>
                );
              })}
      ...
      {addEditOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 110 }}>
          <div style={{ background: '#c0c0c0', ...raised, width: '440px', maxHeight: '90vh', position: 'relative', display: 'flex', flexDirection: 'column' }}>
            <Win95TitleBar title={editingId ? "Modifier le festival" : "Nouveau Festival"} onClose={() => setAddEditOpen(false)} />
            <form onSubmit={e => {
              e.preventDefault();
