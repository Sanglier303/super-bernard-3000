...
export function LieuxWindow({ lieux, loading, saveLieux, onRefresh }) {
  ...
  const handleDelete = async () => {
    ...
    const updated = lieux.map(l => l.id === selected.id ? { ...l, archive: "true" } : l);
    setSelected(null);
          ...
          <div style={{ overflowX: 'auto', flex: 1, display: 'flex', flexDirection: 'column', ...sunken, background: '#fff' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
              {loading ? (
                ...
                return (
                  <div key={l.id} onDoubleClick={() => { setSelected(l); setEditingId(l.id); setAddEditOpen(true); }} onClick={() => setSelected(l)}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 110 }}>
          <div style={{ background: '#c0c0c0', ...raised, width: '420px', maxHeight: '90vh', position: 'relative', display: 'flex', flexDirection: 'column' }}>
            ...
            <form onSubmit={e => {
              ...
              let updated;
              if (editingId) {
                updated = lieux.map(l => l.id === editingId ? { ...l, ...data } : l);
              } else {
                updated = [...lieux, { id: Date.now() + Math.random().toString(), ...data }];
              }
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr', gap: '6px', alignItems: 'center' }}>
                {[['Nom :', 'nom'], ['Capacité :', 'capacite'], ['Adresse :', 'adresse'], ['Instagram :', 'instagram'], ['Photo (URL) :', 'photo']].map(([label, name]) => (
                  <React.Fragment key={name}>
                    ...
                    <input name={name} defaultValue={editingId ? lieux.find(l => l.id === editingId)?.[name] : ''} style={{ ...sunken, padding: '2px 4px', ...winFont, border: 'none' }} />
                  </React.Fragment>
                ))}
                <label style={winFont}>Type :</label>
                <select name="type" defaultValue={editingId ? lieux.find(l => l.id === editingId)?.type : ''} style={{ ...sunken, padding: '2px 4px', ...winFont, border: 'none' }}>
                  <option value="">— choisir —</option>
                  {TYPE_LABELS.map(t => <option key={t} value={t}>{t}</option>)}
                </select>
                <label style={winFont}>Notes :</label>
                <textarea name="notes" defaultValue={editingId ? lieux.find(l => l.id === editingId)?.notes : ''} style={{ ...sunken, padding: '2px 4px', ...winFont, border: 'none', height: '60px' }} />
              </div>
